...
def newQuestion(kbInRule, ruleObjs, varPos, qObjs):
    ...
    print("rule cho cau hoi moi:", kbInRule.name, kbInRule.objs)
    ...
    for i in range(len(kbInRule.objs)):
        ...
        if idx != -1:  # Nếu tìm được thì lưu vào newObjs
            ...
            newVar.append(1 if varPos == idx else 0)
...
def solveStackQuestions(rules, facts, result, questions: [SingleQuestion]):
    ...
    return combineResult(
        rt, [0] * (len(rt) - 1), oldQ.objs[oldQ.pos] if oldQ.pos != -1 else "T/F"
    )
...
def combineResult(result, operator, expect):
    ...
    for i in range(len(result)):
        ...
        for j in range(i + 1, len(result)):
            if rt[-1][0] == result[j][0]:
                rt[-1] = combineOne(operator[i], [rt[-1], result[j]])
...
#     question.xuat()
print(solveSingleQuestion(rules, facts, questions[i]))
